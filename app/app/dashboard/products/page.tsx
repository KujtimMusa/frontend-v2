...
export default function AppDashboardProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (statusFilter !== 'all') filtered = filtered.filter((p) => (p.status || 'optimal') === statusFilter);
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'name': return a.title.localeCompare(b.title);
        case 'price_asc': return a.price - b.price;
        case 'price_desc': return b.price - a.price;
        case 'margin_asc': return (a.cost || 0) - (b.cost || 0);
        case 'margin_desc': return (b.cost || 0) - (a.cost || 0);
        default: return 0;
      }
    });
    return filtered;
  ...
  const handleExport = () => {
    ...
    const csv = [
      ...
      ...filteredProducts.map((p) =>
        [p.title, p.price.toFixed(2), p.cost?.toFixed(2) || '', p.cost ? `${(((p.price - p.cost) / p.price) * 100).toFixed(1)}%` : '', p.status || 'optimal'].join(',')
      ),
    ].join('\n');
    ...
    <div className="space-y-6">
      ...
      {selectedIds.length > 0 && (
        <div className="p-4 bg-muted rounded-lg flex items-center justify-between">
          <span className="text-sm">{selectedIds.length} Produkt{selectedIds.length !== 1 ? 'e' : ''} ausgewählt</span>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setSelectedIds([])}>Auswahl aufheben</Button>
            <Button variant="outline" size="sm">Bulk-Aktion</Button>
          </div>
        </div>
      )}
      ...
      <ProductTable
        ...
        showCheckboxes={true}
        onProductClick={(p) => router.push(`/app/dashboard/recommendations/${p.id}`)}
      />
    </div>
  );
}
